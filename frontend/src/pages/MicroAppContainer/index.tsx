...
const MicroAppContainer = () => {
  ...
  useEffect(() => {
    const fetchApp = async () => {
      ...
      try {
        setLoading(true)
        const appData = await getApplicationsBasicInfo(appKey)
        if (!appData) {
          setError('获取应用配置失败')
        } else {
          ...
          setCurrentMicroApp({
            ...appData,
            routeBasename: getFullPath(`/application/${appData.key}`),
          })
  ...
  }, [appKey, clearCurrentMicroApp, setCurrentMicroApp, setHomeRoute, homeRoute])
